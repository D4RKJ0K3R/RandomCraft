...
public class Incendiary_Enchantment extends CustomWrapper {

    private static HashMap<UUID, Integer> buffer;

    public Incendiary_Enchantment() {
        super("incendiary", 1, 5, new ArrayList<>(), EnchantmentTarget.CROSSBOW, EnchantmentTarget.BOW);
    ...
    private void event(ProjectileLaunchEvent event) {
        if (event.getEntity().getShooter() instanceof Player) {
            ...
            if (CustomEnchants.canUseEnchant(tool, this)) {
                ...
                ((Arrow) event.getEntity()).setColor(Color.ORANGE);
